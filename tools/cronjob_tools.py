...
def cronjob(
    ...
    suppress_unchanged: Optional[bool] = None,
) -> str:
    ...
    try:
        ...
        if normalized == "create":
            ...
            job = create_job(
                ...
                suppress_unchanged=bool(suppress_unchanged),
            )
        ...
        if normalized == "update":
            ...
            if suppress_unchanged is not None:
                updates["suppress_unchanged"] = bool(suppress_unchanged)
            if workdir is not None:
                # Empty string clears the field (restores old behaviour);
                # otherwise pass raw — update_job() validates / normalizes.
                updates["workdir"] = _normalize_optional_job_value(workdir) or None
            if max_iterations is not None:
                # Zero is an update-only clear sentinel. create_job() still
                # requires a positive value when the field is supplied.
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "max_iterations": {
                "type": "integer",
                "minimum": 0,
                "description": "Optional per-job ceiling on model iterations. The scheduler uses the lower of this value and global agent.max_turns, so a job can tighten but never widen operator policy. Ignored for no_agent jobs. On update, pass 0 to clear the override; create requires a value of at least 1."
            },
            "suppress_unchanged": {
                "type": "boolean",
                "default": False,
                "description": "When true, skip delivering output identical to this job's last delivered message (post only on state change). Recommended for recurring status/health/watchdog jobs to avoid reposting unchanged lines. Default false = deliver every tick (no change to existing jobs). On update, pass false to turn it back off."
            },
            "attach_to_session": {
                "type": "boolean",
...
registry.register(
    ...
    handler=lambda args, **kw: (lambda _mo=_resolve_model_override(args.get("model")): cronjob(
        ...
        suppress_unchanged=args.get("suppress_unchanged"),
