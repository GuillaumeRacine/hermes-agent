...
def get_secret_source(env_var: str) -> str | None:
    ...
    Returns a source slug for keys pulled from an external secret manager
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    if op_cfg.get("enabled"):
        try:
            ...
            apply_onepassword_secrets = None

        if apply_onepassword_secrets is not None:
            op_result = apply_onepassword_secrets(
                enabled=True,
            ...
            if op_result.applied:
                _sanitize_loaded_credentials()
                for name in op_result.applied:
                    _SECRET_SOURCES[name] = "1password"
                print(
                    f"  1Password: applied {len(op_result.applied)} "
                    f"secret{'s' if len(op_result.applied) != 1 else ''} "
                    f"({', '.join(sorted(op_result.applied))})",
                    file=sys.stderr,
                )
            if op_result.error:
                print(f"  1Password: {op_result.error}", file=sys.stderr)
            for warn in op_result.warnings:
                print(f"  1Password: {warn}", file=sys.stderr)
