...
| `SLACK_DM_POLICY` | Slack direct-message intake policy: `open` (default) or `disabled`. `disabled` drops 1:1 and multi-person DMs before session creation. Equivalent to `slack.dm_policy` in `config.yaml`. |
